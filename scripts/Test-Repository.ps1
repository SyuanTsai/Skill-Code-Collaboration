...
if ($inventory.sourceId -isnot [string] -or [string]$inventory.sourceId -cne 'code-collaboration') {
    throw "catalog/source.json sourceId must be exact string 'code-collaboration'."
}
if ($inventory.repository -isnot [string] -or [string]$inventory.repository -cne 'https://github.com/SyuanTsai/Skill-Code-Collaboration.git') {
...
Write-Host "Code Collaboration repository validation passed: $($skillIds.Count) active Skills."
$json
