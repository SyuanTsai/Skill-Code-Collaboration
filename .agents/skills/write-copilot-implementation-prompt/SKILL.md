...
3. Recommend the lowest-capability currently available Copilot model that can complete the task reliably with reasonable quota usage:
   ...
   - Never substitute a capability tier, model family, generic description, or exclusion-only guidance for the exact model name.
...
4. Make the prompt directly executable and include objective, relevant files or symbols, scope and exclusions, implementation constraints, acceptance criteria, test requirements, validation expectations, and a completion report.
