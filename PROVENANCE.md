...
- Canonical source inventory: `catalog/source.json` with source ID `code-collaboration`.
- Standard authority: `SyuanTsai-AI-Instructions` commit `5ff96a358a51788a3764b27c31def842d5aee55d`, with the immutable archive hash recorded in `config/standard-v1.json`.
- Package integrity and per-Skill content hashes are generated by `scripts/Test-Repository.ps1` and consumed by `scripts/Validate.ps1`; the domain profile catalog remains selection metadata rather than a second validation policy.
